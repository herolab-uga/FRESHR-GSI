...
class Yolov7Publisher:
    ...
    def process_img_msg(self, img_msg: Image):
        ...
        np_img_orig = self.bridge.imgmsg_to_cv2(img_msg, desired_encoding='passthrough')
        ...
        bboxes_pub = rospy.Publisher('/bboxes_array', Float64MultiArray, queue_size = 10)
        ...
        if self.visualization_publisher:
            ...
            print(classes)
            my_bbox.data = [float(bboxes[0][0]),float(bboxes[0][1]),float(bboxes[0][2]),float(bboxes[0][3]),float(496),float(239),float(593),float(271)]
            bboxes_pub.publish(my_bbox)
